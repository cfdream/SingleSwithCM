...
public class MainTask {
	...
	public static void RunIntervalsChangeLossRatioThreshold()
	{
		...
		for (int ithInterval = 0; ithInterval < GlobalData.SIMULATE_INVERVALS; ithInterval++) {
			...
			for (GlobalData.METHOD_NUMBER = startMethod; GlobalData.METHOD_NUMBER <= endMethod; GlobalData.METHOD_NUMBER++) {
				for (int i = 1; i <= 4; i++) {
					...
					System.out.println("loss rate:" + GlobalData.TARGET_FLOW_LOST_RATE_THRESHOLD);
					Switch switch1 = new Switch();
					switch1.runDataInList();
					switch1.sendDataToControllerAndClearStatus();
					ResultData resultData = new ResultData(0, 0, 0, 0, 0);
					...
					HashMap<Integer, ArrayList<ResultData>> listResultDataMap = listMethodResultDataMap.get(GlobalData.METHOD_NUMBER);
					if (listResultDataMap == null) {
						listResultDataMap = new HashMap<Integer, ArrayList<ResultData>>();
						listMethodResultDataMap.put(GlobalData.METHOD_NUMBER, listResultDataMap);
					}
		...
		for (GlobalData.METHOD_NUMBER = startMethod; GlobalData.METHOD_NUMBER <= endMethod; GlobalData.METHOD_NUMBER++) {
			...
			GlobalData.RESULT_FILE_NAME = "data\\intervalResults"  
					+ "Volume_threshold_" + GlobalData.NORMAL_VOLUME_THRESHOLD_FOR_COMPUTE_LOSS_RATIO
					+ "_DiffLossRatioMethod"+ GlobalData.METHOD_NUMBER + ".txt";
			HashMap<Integer, ArrayList<ResultData>> listResultDataMap = listMethodResultDataMap.get(GlobalData.METHOD_NUMBER);
			try {
				BufferedWriter writer;
				writer = new BufferedWriter(new FileWriter(GlobalData.RESULT_FILE_NAME, true));
	...
	public static void RunIntervalsChangeLossRatioThresholdChangeCalculateVolumeForMethod2()
	{
		...
		for (int ithInterval = 0; ithInterval < GlobalData.SIMULATE_INVERVALS; ithInterval++) {
			...
			for (int k = 0; k < volumeThresholds.length; ++k) {
				...
				for (int i = 1; i <= 4; i++) {
					...
					HashMap<Integer, ArrayList<ResultData>> listResultDataMap = listMethodResultDataMap.get(GlobalData.NORMAL_VOLUME_THRESHOLD_FOR_COMPUTE_LOSS_RATIO);
					if (listResultDataMap == null) {
						listResultDataMap = new HashMap<Integer, ArrayList<ResultData>>();
						listMethodResultDataMap.put(GlobalData.NORMAL_VOLUME_THRESHOLD_FOR_COMPUTE_LOSS_RATIO, listResultDataMap);
					}
		...
		for (int k = 0; k < volumeThresholds.length; ++k) {
			...
			try {
				BufferedWriter writer;
				writer = new BufferedWriter(new FileWriter(GlobalData.RESULT_FILE_NAME, true));
				writer.write("normal volume threshold for compute loss ratio:" + GlobalData.NORMAL_VOLUME_THRESHOLD_FOR_COMPUTE_LOSS_RATIO + "\n");
				writer.close();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	public static void RunIntervalsChangeWaitTimeMethod3()
	{
		...
		for (int ithInterval = 0; ithInterval < GlobalData.SIMULATE_INVERVALS; ithInterval++) {
			...
			for (int i = 2; i <= 6; i+=2) {
				GlobalData.NUMBER_MICSECONDS_TO_WAIT_BEFORE_DELETE = Math.pow(10, i);
				System.out.println("number of micseconds to wait:" + GlobalData.NUMBER_MICSECONDS_TO_WAIT_BEFORE_DELETE);
				
				HashMap<Integer, ArrayList<ResultData>> listResultDataMap = listMethodResultDataMap.get(i);
				if (listResultDataMap == null) {
					listResultDataMap = new HashMap<Integer, ArrayList<ResultData>>();
				...
				for (int j = 1; j <= 4; j++) {
					...
					ArrayList<ResultData> listResultDatas = listResultDataMap.get(j);
					if (listResultDatas == null) {
						listResultDatas = new ArrayList<ResultData>();
		...
		for (int i = 2; i <= 6; i+=2) {
			GlobalData.RESULT_FILE_NAME = "data\\intervalResults"  
					+ "waitTime_" + Math.pow(10, i)
					+ "_"+ GlobalData.METHOD_NUMBER + ".txt";
			try {
				BufferedWriter writer;
				writer = new BufferedWriter(new FileWriter(GlobalData.RESULT_FILE_NAME, true));
				writer.write("number_of_microseconds_to_wait:" + Math.pow(10, i) + "\n");
				writer.close();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			...
			for (int j = 1; j <= 4; j++) {
				...
				try {
					BufferedWriter writer;
					writer = new BufferedWriter(new FileWriter(GlobalData.RESULT_FILE_NAME, true));
					writer.write("loss_rate:" + GlobalData.TARGET_FLOW_LOST_RATE_THRESHOLD + "\n");
					writer.close();
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
...
//		FixSizeHashMap.ARRAY_SIZE = 240007;
		...
		String[] modelNamesStrings={"exponential", "polynomial", "log", "linear", "traditional"};
		HashMap<Integer, ArrayList<ResultData>> listResultDataMap = new HashMap<Integer, ArrayList<ResultData>>();
		...
		for (int ithInterval = 0; ithInterval < GlobalData.SIMULATE_INVERVALS; ithInterval++) {
			...
			for (int i = 0; i < 1; i++) {
				...
				Switch switch1 = new Switch();
				if (0 == i) {
					switch1.packetSampleModel = 
							new PacketSampleModelExponential(switch1.lostFlowVolumeMap,
									switch1.normalFlowVolumeMap,
									switch1.sampledFlowVolumeMap);
				} else if (1==i) {
					switch1.packetSampleModel = 
							new PacketSampleModelPolynomial(switch1.lostFlowVolumeMap, 
									switch1.normalFlowVolumeMap,
									switch1.sampledFlowVolumeMap);
				}else if (2==i) {
					switch1.packetSampleModel = 
							new PacketSampleModelLog(switch1.lostFlowVolumeMap, 
									switch1.normalFlowVolumeMap,
									switch1.sampledFlowVolumeMap);
				}else if (3==i) {
					switch1.packetSampleModel = 
							new PacketSampleModelLinear(switch1.lostFlowVolumeMap, 
									switch1.normalFlowVolumeMap,
									switch1.sampledFlowVolumeMap);
				} else if(4==i) {
					switch1.packetSampleModel = 
							new PacketSampleModelTraditional(switch1.lostFlowVolumeMap, 
									switch1.normalFlowVolumeMap,
									switch1.sampledFlowVolumeMap);
				}
				switch1.runDataInList();
		...
		GlobalData.RESULT_FILE_NAME = "data\\intervalResults"  
				+ "_DiffModel__prob_" + GlobalData.DEAFULT_BYTE_SAMPLE_RATE + ".txt";
		for (int i = 0; i < 5; i++) {			
			ArrayList<ResultData> listResultDatas = listResultDataMap.get(i);
			try {
				BufferedWriter writer;
	...
	public static void main(String[] args) {
		...
		//DataAnalysis.analyzeFlowNormalLostPacketsDistribution(flowKey, 100);	//10ms per interval
		
		//DataAnalysis.analyzeFlowDataDistributionInOneInterval();
		
		
		/*
		 * Read one interval data, then calculate multiple times based on the data
		 * then, read next interval data, then calculate...
		 
		Reader reader = new Reader();
		Switch switch1 = new Switch();
		ResultData[][] resultDatas = new ResultData[GlobalData.DEFAULT_DELTA_BYTE_SAMPLE_RATE_INCREASE_TIMES][GlobalData.SIMULATE_INVERVALS];
		
		for (int i = 0; i < GlobalData.SIMULATE_INVERVALS; i++) {
			//clear listPackets for this SIMULATE_INTERVAL
			Switch.listPackets.clear();
			
			//read packets belonging to this SIMULATE_INTERVAL
			if (reader.getIthIntervalData(i) != 0) {
				System.out.print("read data for " + i + "th interval failed");
				break;
			}
			
			//do experiments for changing DEFAULT_DELTA_BYTE_SAMPLE_RATE
			for (int j = 0; j < GlobalData.DEFAULT_DELTA_BYTE_SAMPLE_RATE_INCREASE_TIMES; j++) {
				GlobalData.DEFAULT_DELTA_BYTE_SAMPLE_RATE *= Math.pow(10, j);
				switch1.runOneInterval();
				switch1.sendDataToControllerAndClearStatus();
				
				//do statistics for the <ith interval, certain delta_byte_sampling_rate> pair
				//clear all data in Controller
				ResultData resultDataOneInterval = new ResultData(0, 0);
				Controller.analyze(resultDataOneInterval);
				Controller.clear();
		...
		*/
		...
		for (GlobalData.DEFAULT_DELTA_BYTE_SAMPLE_RATE = 1.7e-10; 
			...
			for(int i = 0; i < tryTimes; ++i){		
				Controller.SAMPLED_NORMAL_PACKET_MAP.clear();
				ResultData resultData = new ResultData(0, 0, 0);
				Switch switch1 = new Switch();
				switch1.run2(resultData);
			...
			double totalError = 0;
			for (ResultData resultData : listResultData) {
				totalFalsePositive += resultData.falsePositive;
				totalCoverage += resultData.targetFlowPacketNumCoverage;
				totalError += resultData.heldFlowFalsePacketNumCoverage;
			}
			
			double avgFalsePositive = totalFalsePositive / tryTimes;
			double avgCoverage = totalCoverage / tryTimes;
			double avgError = totalError / tryTimes;
		...
		*/
		...
		/*
		HashMap<Packet, Integer> map = new HashMap<Packet, Integer>();
		Packet pkg = new Packet(Long.MIN_VALUE, Long.MIN_VALUE, Long.MIN_VALUE, 0, 0, (short) 0, 0, Packet.MSG_END);
		map.put(pkg, 1);
		Packet pkg2 = new Packet(Long.MIN_VALUE, Long.MIN_VALUE, Long.MIN_VALUE, 0, 0, (short) 0, 0, Packet.MSG_END);
		System.out.println(map.get(pkg2));
		*/
	}
