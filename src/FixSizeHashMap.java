...
public class FixSizeHashMap {
	public static int ARRAY_SIZE = 1200007; //10007 105943, 1000003, 1200007(1.2M) 13567(13k) 240007(240k)
	...
	public void put(FlowKey flowKey, long value, Long timestamp, HashMap<FlowKey, Long> lostFlowVolumeMap) {
		...
		if (1 == GlobalData.OBJECT_VOLUME_OR_RATE) {
			//------target is volume 
			double avgLossSpeed = GlobalData.TARGET_FLOW_LOST_VOLUME_THRESHOLD / GlobalData.INTERVAL_SECONDS;
			double lossSpeed = lostVolume / (1.0 * (timestamp - starttime) / GlobalData.SECOND_2_USECOND);
			...
			if(GlobalData.METHOD_NUMBER == 3){
				if (timestamp - starttime > GlobalData.NUMBER_MICSECONDS_TO_WAIT_BEFORE_DELETE
					...
					//the flow occupy the bucket larger than NUMBER_SECONDS_TO_WAIT_BEFORE_DELETE, discard it.
					entries[idx] = newEntry;
					return;
				}
			}
			if (GlobalData.METHOD_NUMBER == 4) {
				...
				if (timestamp - starttime > 
						GlobalData.SECOND_2_USECOND * GlobalData.NUMBER_MICSECONDS_TO_WAIT_BEFORE_DELETE
						&& lossRate < GlobalData.TARGET_FLOW_LOST_RATE_THRESHOLD) {
					//3.3 totalVolume for existing flow is not big enough
					//the flow occupy the bucket larger than NUMBER_SECONDS_TO_WAIT_BEFORE_DELETE, discard it.
					entries[idx] = newEntry;
