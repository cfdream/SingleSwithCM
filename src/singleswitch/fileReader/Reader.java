package singleswitch.fileReader;
...
public class Reader {
	...
	/*
	 * ithInterval: [0, SIMULATE_INTERVALS) TODO: to test
	 * 
	...
	public int readIthIntervalPackets(int ithInterval) {
		...
		// try {
		// lostWriter = new BufferedWriter(new
		// FileWriter("data\\lostPacketsInOneInterval.txt"));
		//
	...
	/*
	 * read all packet information from filePath randomly drop packets, and add
	 * packet headers (both normal and loss packets) into QUEUE when the file is
	 * finished read, add END packet header into QUEUE
	 */
	/*
	 * public void readOneFile(String filePath) { Random rand = new Random();
	 * Long interval_stime = Long.MIN_VALUE; int seconds = 0; int
	 * packetSizeInOneSec = 0; //header + body int lines = 0;
	 * 
	 * try { reader = new BufferedReader(new FileReader(filePath));
	 * 
	 * String line; while ((line = reader.readLine()) != null) { String[]
	 * subStrs = line.split(","); if (subStrs.length != 7) { continue; } Long
	 * microsec = Long.parseLong(subStrs[0]); Long srcip =
	 * Long.parseLong(subStrs[1]); Long destip = Long.parseLong(subStrs[2]); int
	 * srcport = Integer.parseInt(subStrs[3]); int destport =
	 * Integer.parseInt(subStrs[4]); if(subStrs[5].equals("null")){ continue; }
	 * short protocol = Short.parseShort(subStrs[5]); int length =
	 * Integer.parseInt(subStrs[6]); //body size length += Packet.HEADER_SIZE;
	 * //total packet size
	 * 
	 * lines++; packetSizeInOneSec += length; if (interval_stime ==
	 * Long.MIN_VALUE) { interval_stime = microsec; } if ((microsec -
	 * interval_stime) > 1000000) { seconds++;
	 * System.out.println("Reader:"+seconds
	 * +"s, lines:"+lines+", flows:"+Controller.getFlowSize()
	 * +",packetSizeInOneSec:"+packetSizeInOneSec);
	 * 
	 * //----send the collected data to Controller interval_stime = microsec;
	 * packetSizeInOneSec = 0; } if (seconds >= GlobalData.SIMULATE_SECONDS) {
	 * break; }
	 * 
	 * //calculate prob of dropping the packet double prob = rand.nextDouble();
	 * if (prob > GlobalData.PACKET_LOSS_RATE) { //----normal packet Packet pkg
	 * = new Packet(microsec, srcip, destip, srcport, destport, protocol,
	 * length, Packet.MSG_NORMAL); Switch.PACKET_QUEUE.put(pkg);
	 * Switch.listPackets.add(pkg);
	 * 
	 * //get the flow for the packet FlowKey flow = new FlowKey(pkg);
	 * 
	 * //ground truth of normal packet Controller.addNormalPacketsForFlow(flow,
	 * 1); } else { //----lost packet Packet pkg = new Packet(microsec, srcip,
	 * destip, srcport, destport, protocol, length, Packet.MSG_LOST);
	 * Switch.PACKET_QUEUE.put(pkg); Switch.listPackets.add(pkg);
	 * 
	 * //get the flow for the packet FlowKey flow = new FlowKey(pkg);
	 * 
	 * //ground truth of lost packet Controller.addLostPacketsForFlow(flow, 1);
	 * } }//end while ((line = reader.readLine()) != null) reader.close(); }
	 * catch (FileNotFoundException e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); } catch (IOException e) { // TODO Auto-generated
	 * catch block e.printStackTrace(); } catch (InterruptedException e) { //
	 * TODO Auto-generated catch block e.printStackTrace(); }
	 * 
	 * //insert the END signal to the queue Packet pkg = new
	 * Packet(Long.MIN_VALUE, Long.MIN_VALUE, Long.MIN_VALUE, 0, 0, (short) 0,
	 * 0, Packet.MSG_END); try { Switch.PACKET_QUEUE.put(pkg);
	 * Switch.listPackets.add(pkg); } catch (InterruptedException e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); } }
	 * 
	 * public void run(){
	 * 
	 * String filePath =
	 * "C:\\workspace\\data\\equinix-sanjose.dirA.20120920-130000.UTC.anon.pcap.csv"
	 * ;
	 * 
