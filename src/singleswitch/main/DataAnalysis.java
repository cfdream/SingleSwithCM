...
public class DataAnalysis {
	...
	public static void analyzeListIntervalResults(
			ArrayList<ResultData> listResultDatas) {
		...
		for (Iterator<ResultData> iterator = listResultDatas.iterator(); iterator
				.hasNext();) {
			ResultData resultData = iterator.next();
			standardDeviationMemoryPrecision += Math.pow(
					resultData.falsePositive - avgFalsePositive, 2);
			standardDeviationFalseNegative += Math.pow(resultData.falseNegative
					- avgFalseNegative, 2);
			standardDeviationAccuracy += Math.pow(resultData.accuracy
					- avgAccuracy, 2);
		}
		standardDeviationMemoryPrecision = Math.pow(
				standardDeviationMemoryPrecision / numResults, 0.5);
		standardDeviationFalseNegative = Math.pow(
				standardDeviationFalseNegative / numResults, 0.5);
		standardDeviationAccuracy = Math.pow(standardDeviationAccuracy
				/ numResults, 0.5);
		...
		try {
			...
			System.out.println("avgFalsePositive:" + avgFalsePositive + " "
					+ "avgFalseNegative:" + avgFalseNegative + " "
					+ "avgAccuracy:" + avgAccuracy + "\r\n");
		} catch (IOException e) {
	 ...
	 */
	public static void analyzeFlowNormalLostPacketsDistribution(
			FlowKey flowKey, int numMillseconds) {
		try {
			BufferedReader lostReader = new BufferedReader(new FileReader(
					"data\\lostPacketsInOneInterval.txt"));
			BufferedReader normalReader = new BufferedReader(new FileReader(
					"data\\normalPacketsInOneInterval.txt"));
			String line = null;
			BufferedWriter flowLostWriter = new BufferedWriter(new FileWriter(
					"data\\" + flowKey.srcip + "_lost.txt"));
			BufferedWriter flowNormalWriter = new BufferedWriter(
					new FileWriter("data\\" + flowKey.srcip + "_normal.txt"));
			...
			while (intervalStart < END) {
				...
				flowLostWriter
						.write(intervalStart + " " + lostVolume + " \n\r");
	...
	public static void analyzeFlowDataDistributionInOneInterval() {
		...
		for (Entry<FlowKey, FlowValue> pair : Controller.FLOW_MAP.entrySet()) {
			...
			listFlowAvgPackgeLength.add(1.0 * flowValue.volume
					/ flowValue.numAllPackets);
		...
		System.out.println("totalVolumeForFlowWithPkgLargerThan1k:"
				+ totalFlowVolumePkgNum1k);
		System.out.println("totalPacketNumForFlowWithPkgLargerThan1k:"
				+ totalFlowPacketsPkgNum1k);
		System.out.println("totalFlowNumForVolume100k:"
				+ totalFlowNumForVolume100k + ", totalFlowVolumeForVolume100k:"
				+ totalFlowVolumeForVolume100k);
		...
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(
					"data\\analyzeFlowPacketNumDistributionInOneInterval.txt"));
			for (double i = 0; i <= 1; i += 0.0001) {
				...
				int packetNumForTheFlow = listFlowAllPacketNum
						.get(percentIndex);
			...
			writer = new BufferedWriter(new FileWriter(
					"data\\analyzeFlowVolumeDistributionInOneInterval.txt"));
			for (double i = 0; i <= 1; i += 0.0001) {
			...
			writer = new BufferedWriter(new FileWriter(
					"data\\analyzeFlowAvgPacketLength.txt"));
			for (double i = 0; i <= 1; i += 0.0001) {
				...
				Double avgPacketLength = listFlowAvgPackgeLength
						.get(percentIndex);
			...
			writer = new BufferedWriter(new FileWriter(
					"data\\analyzeFlowLostVolume.txt"));
			for (double i = 0; i <= 1; i += 0.0001) {
			...
			writer = new BufferedWriter(new FileWriter(
					"data\\analyzeFlowVolume_lossRate.txt"));
			long preVolume = 0;
			long cnt = 0;
			...
			for (Iterator<FlowValue> iterator = listFlowVolumeLostRate
				...
				if (flowValue.volume != preVolume) {
					// new volume value
					if (preVolume != 0) {
						writer.write(preVolume + " " + sumLossRate + " " + cnt
								+ " " + sumLossRate / cnt + "  \n\r");
					}
