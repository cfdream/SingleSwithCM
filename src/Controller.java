...
public class Controller {
	...
	/*
	//normal flow map
	public static ConcurrentHashMap<FlowKey, Integer> NORMAL_FLOW_MAP = new ConcurrentHashMap<FlowKey, Integer>();
	//sampled normal flow map 
	public static ConcurrentHashMap<FlowKey, Integer> SAMPLED_NORMAL_FLOW_MAP = new ConcurrentHashMap<FlowKey, Integer>();
	//lost flow map
	public static ConcurrentHashMap<FlowKey, Integer> LOST_FLOW_MAP = new ConcurrentHashMap<FlowKey, Integer>();
	*/
	...
	public static void clearSwitchData() {
		for (Iterator<Entry<FlowKey, FlowValue>> iterator = FLOW_MAP.entrySet().iterator(); iterator.hasNext();) {
	...
	public static void analyze(ResultData resultData) {
		...
		//4. 
		try {
			BufferedWriter flowMonitorWriter = 
					new BufferedWriter(new FileWriter("data\\flowsToMonitor.txt"));
			...
			for (Map.Entry<FlowKey, FlowValue> entry : FLOW_MAP.entrySet()) {
				...
				if (1 == GlobalData.OBJECT_VOLUME_OR_RATE) {
					...
					//lost rate
					double rate = 1.0 * flowValue.lostVolume / (flowValue.lostVolume + flowValue.normalVolume);
				...
				if (flowValue.sampledNormalVolume == 0) {
					...
					flowMonitorWriter.write("false negative:" + entry.getKey().srcip
											+"\r\n");
				} else {
					//held
					targetFlowHeldNum += 1;
			...
			//memory inflation
			double memoryInflation = 1.0 *  (totalHeldFlowNum - targetFlowHeldNum)/ totalHeldFlowNum;
			...
			System.out.println("normal table size: " + FixSizeHashMap.ARRAY_SIZE);
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter("data\\analyzeFlowVolume_accuracy.txt"));
			long preVolume = 0;
			long cnt = 0;
			...
			for (Iterator<FlowValue> iterator = listFlowVolumeLostRate.iterator(); iterator.hasNext();) {
				...
				if (flowValue.volume != preVolume) {
					//new volume value
					if (preVolume != 0) {
						writer.write(preVolume + " " + sumAccuracy + " " + cnt + " " + sumAccuracy / cnt + "  \n\r");
					}
...
/*	public static void analyze(ResultData resultData) {
		//get ground truth target flows
		int totalPacketLost = 0;
		for(Map.Entry<FlowKey, Integer> entry : LOST_PACKET_MAP.entrySet()) {
			totalPacketLost += entry.getValue();
		}
		//average packet lost for lost flows
		double avgPacketLost = 1.0 * totalPacketLost / LOST_PACKET_MAP.size();
		
		HashSet<Flow> targetFlows = new HashSet<Flow>();
		int totalTargetFlowsPacketNum = 0;
		for(Map.Entry<FlowKey, Integer> entry : LOST_PACKET_MAP.entrySet()) {
			Flow pkg = entry.getKey();
			Integer lossCnt = entry.getValue();
			if (lossCnt < GlobalData.LEAST_LOSS_PACKET_IN_ONE_SECOND) {
				continue;
			}
			
			targetFlows.add(pkg);
			totalTargetFlowsPacketNum += NORMAL_PACKET_MAP.get(pkg);
		}
		
		//----false positive
		int numFalsePosFlows = 0;
		int totalFalsePosFlowsPacketNum = 0;
		int totalHeldFlowPacketNum = 0;
		for(Map.Entry<FlowKey, Integer> entry : SAMPLED_NORMAL_PACKET_MAP.entrySet()){
			totalHeldFlowPacketNum += entry.getValue();
			if (!targetFlows.contains(entry.getKey())) {
				//held flows not exist in targetFlows
				++numFalsePosFlows;
				totalFalsePosFlowsPacketNum += entry.getValue();
			}
		}
		
		double falsePosRate = 1.0 * numFalsePosFlows / SAMPLED_NORMAL_PACKET_MAP.size();
		
		int totalSuccHeldFlowPacketNum = totalHeldFlowPacketNum - totalFalsePosFlowsPacketNum;
		
		//----false negative
		int numCorrectSampledFlows = SAMPLED_NORMAL_PACKET_MAP.size() - numFalsePosFlows;
		int numFalseNegFlows = targetFlows.size() - numCorrectSampledFlows;
		double falseNegRate = 1.0 * numFalseNegFlows / targetFlows.size();
		
		//----accuracy
		for (Iterator iterator = targetFlows.iterator(); iterator.hasNext();) {
		...
		//----average packet held rate for the held flows
		double totalRate = 0;
		double numFlows = 0;
		ArrayList<Double> listRate = new ArrayList<Double>();
		int totalHeldCnt = 0;
		int totalTruthCnt = 0;
		for(Map.Entry<Packet, Integer> entry : SAMPLED_NORMAL_PACKET_MAP.entrySet()){
			Packet pkg = entry.getKey();
			if (!targetFlows.contains(pkg)){
				//sampled, but not targeted flows
				continue;
			}
			
			int heldCnt = entry.getValue();
			int groundTruthCnt = NORMAL_PACKET_MAP.get(pkg);
			double rate = 1.0 * heldCnt / groundTruthCnt;
			totalRate += rate;
			++numFlows;
			listRate.add(rate);
			totalHeldCnt += heldCnt;
			totalTruthCnt += groundTruthCnt;
		}
		double avgHeldRate = totalRate / numFlows;
		
		double totalError = 0;
		for (Iterator<Double> iterator = listRate.iterator(); iterator.hasNext();) {
			double rate = (Double)iterator.next();
			totalError += Math.pow(rate - avgHeldRate, 2);
		}
		...
		System.out.println("#average lost packet among lost flows:" + avgPacketLost);
		System.out.println("#packet lost of target flows:" + GlobalData.LEAST_LOSS_PACKET_IN_ONE_SECOND);
		System.out.println("#target flow:" + targetFlows.size());
		System.out.println("#successful targetted flow:" + (SAMPLED_NORMAL_PACKET_MAP.size() - numFalsePosFlows));
		System.out.println("#flow sampled:" + SAMPLED_NORMAL_PACKET_MAP.size());
		System.out.println("#flows false positive:" + numFalsePosFlows + ",false positive rate:" + falsePosRate);
		System.out.println("#flows false positive:" + numFalseNegFlows + ",false negative rate:"+falseNegRate);
		System.out.println("totalSuccHeldPacketCnt:" + totalHeldCnt + ", totalGroundTruthCntForPreFlows:" + totalTruthCnt);
		System.out.println("average packet held rate for the held flows:" + avgHeldRate + ", standard deviation:" + standardDeviation);
		System.out.println("#SuccHeldTargetFlowPacketNum/#TargetFlowPacketNum:" + 1.0*totalSuccHeldFlowPacketNum/totalTargetFlowsPacketNum);
		System.out.println("#FalsePositiveFlowPacketNum/#HeldFlowPacketNum:" + 1.0*totalFalsePosFlowsPacketNum/totalHeldFlowPacketNum);
		
		resultData.falsePositive = falsePosRate;
		resultData.targetFlowPacketNumCoverage = 1.0*totalSuccHeldFlowPacketNum/totalTargetFlowsPacketNum;
		resultData.heldFlowFalsePacketNumCoverage = 1.0*totalFalsePosFlowsPacketNum/totalHeldFlowPacketNum;
	}*/
}
